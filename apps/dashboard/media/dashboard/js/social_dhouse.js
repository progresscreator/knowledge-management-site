...
color from = color(#b6d5ff);
color to = color(#172141);
...
color ans_qs_color = color(#FC3B52);
color ask_qs_color = color(#5BCD7A);
color both_color = color(#2646CC);
...
color[] egghead_colors = new color[3];
...
void setup() {
	...
	titles[2] = "Student";
	for (int k = 0; k < 6; k++) {
		layer_colors[k] = lerpColor(from, to, 0.2 * k);
	}
	egghead_colors[0] = color(#5BCD7A);
	egghead_colors[1] = color(#2646CC);
	egghead_colors[2] = color(#FC3B52);
	...
	$.getJSON(json_network_url, function(data){
		$.each(data, function(i, egghead){
			...
			new_egghead.link_count = new_egghead.question_count + new_egghead.answer_count - 2;
...
void draw_legend() {
	fill(#522719);
	text("Your Knowledge Gradient", 45, 650);
	for (int k = 0; k < 3; k++) {
...
class Egghead{
	...
	Egghead (String nm, String iu) {
		...
		ans_qs.add(new Question("They knew:", "None"));
		answer_count = answer_count + 1;

		ask_qs = new ArrayList();
		ask_qs.add(new Question("You knew:", "None"));
	...
	void render() {
		...
		if (name != my_name) {	
			pushMatrix();
				...
				if (answer_count > 1 && question_count > 1){
					strokeWeight(answer_count + question_count * 1.5);
					stroke(both_color);
					beginShape();
