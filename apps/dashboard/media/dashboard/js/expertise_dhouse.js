...
color from = color(#FEFFFE);
color to = color(#FFE87C);
int max_tag_count = 0;
...
color marked_color = color(#7D2252);

color tag_normal_text_color = color(#151B8D);
color tag_hover_text_color = color(#ECE5B6);
color question_normal_color = color(#FEFEFE);
color question_hover_color = color(#FFFC17);
...
bool nothingSelected(){
    for (int k=0; k < tags.size(); k++){
	if(k>0){
	    if(tags.get(k).mouseOver || tags.get(k).highlighted)
		return false;
	}
    }
...
void defaultSelectOff(){
    if (tags.size() > 0){
	if(tags.get(0).mouseOver)
	    tags.get(0).mouseOver = false;
    }
}
...
class Tag{
	...
	void render() {
		...
		if (mouseOver || highlighted) {
			...
			for (int k = 0; k < linked_tags.size(); k++) {
				...
				stroke("red");
				tag.current_background_color = color(#E66C2C);
	...
	void rollOver() {
		if (!mouseOver) {      
			mouseOver = true;
			cursor("pointer");
			current_background_color = color(#800517);
...
class Egghead{
	...
	Egghead (int ind, int uid, String nm, String iu, String purl, int wth) {
		...
		profile_url = purl;
		wealth = wth;
		x = tag_panel_width + egghead_panel_width / 2 + bounary_weight - 75;
